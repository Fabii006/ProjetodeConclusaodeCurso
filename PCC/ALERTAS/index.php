    ...
    <body>
        ...
        <div class="VoceChegou">
            <div class="vocechegou-mx">
                <h2 class="hello">Olá, <?php echo $row_usuario['Nome_social'] ?>.</h2>
                ...
                <p class="description description_2">
                    ...
                    do projeto, você pode criar um alerta e esperar que outros usuários te ajudem, caso se tratar
                    de uma vaquinha/arrecadação de dinheiro, opte pelo alerta do tipo doações, se não, crie um do
                    tipo genérico e deixe-o bem informativo!!
                </p>
            </div>
        ...
        <div class="solicitar" id="solicitar">
            <div class="content-meusalertas">
                <?php foreach (solicitar($conex) as $value) {  ?>
                    <div id="card__alertas-id-<?php echo $value['ID'] ?>" class="alertas" onclick="Abrir_alerta_solicitar(<?php echo $value['ID'] ?>)">
                        ...
                        <div class="meusalertas-texto">
                            ...
                            <input type="hidden" name="Nome" value="<?php echo $value['Nome'] ?>" id="card__alertas-nome-<?php echo $value['ID'] ?>">
        ...
        <div class="fornecer" id="fornecer">
            <div class="content-meusalertas">
                <?php foreach (fornecer($conex) as $value) {  ?>
                    <div id="card__alertas-id-<?php echo $value['ID'] ?>" class="alertas" onclick="Abrir_alerta_fornecer(<?php echo $value['ID'] ?>)">
                        <img class="img_alerta" id="card__alertas-img-<?php echo $value['ID'] ?>" src="../IMAGENS_PERFIL/<?php echo $value['Imagem_perfil'] ?>">
                        <div class="meusalertas-texto">
                            <h2 class="title_alerta" id="card__alertas-titulo-<?php echo $value['ID'] ?>"><?= $value['Titulo'] ?></h2>
                            <input type="hidden" name="Nome" value="<?php echo $value['Nome'] ?>" id="card__alertas-nome-<?php echo $value['ID'] ?>">
